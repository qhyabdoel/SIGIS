...
<table>
	...
	<tr>
		...
		<td>
			<?php echo $form->textField($terlambat,'Terlambat_Bulanan',array('disabled'=>'true')); ?>
			<?php echo $form->textField($terlambat,'Terlambat_Bulanan',array('hidden'=>'true')); ?>
	...
	<tr>
		<td>Persetujuan Terlambat</td>
		<td>
			<?php 
			$data 	= array('diproses'=>'diproses','disetujui'=>'disetujui','ditolak'=>'ditolak'); 
			$enable	= array();
			if($role=='admin' or $terlambat->final==1) $enable = array('disabled'=>'true');			
			?>
			<?php echo $form->dropDownList($terlambat,'Status',$data,$enable); ?>
			<?php echo $form->error($terlambat,'Status',array('style'=>'color:#C00000;')); ?>
		</td>
	</tr>	
</table>
