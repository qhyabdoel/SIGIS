...
class TbKetentuanController extends Controller{
	...
	public function accessRules(){
		return array(						
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','disable','index','action','add_masa_kerja','add_golongan'),
				'users'=>array('@'),
			),			
	...
	public function actionCreate(){
		...
		$masa_kerja 		= CHtml::listData(TbMasakerjaKetentuan::model()->findAll(),'masa_kerja','masa_kerja');
		$confirm 			= 0;
	...
	public function actionAdd_masa_kerja(){
		...
		$id 			= '';
		$url 			= 'TbKetentuan/create?action=create';		
		$from 			= 'create';				

		if(isset($_REQUEST['id']) and $_REQUEST['id']!=''){
			$from 	= 'update';
			$id 	= $_REQUEST['id'];
			$url 	= 'TbKetentuan/update/id/'.$id;
		}

		if(isset($_REQUEST['from'])){			
			$masa_kerja_awal 	= $_REQUEST['masa_kerja_awal'];
			$masa_kerja_akhir 	= $_REQUEST['masa_kerja_akhir'];			

			if(is_numeric($masa_kerja_awal) and is_numeric($masa_kerja_akhir)){		
				if(0<=$masa_kerja_awal and $masa_kerja_awal<=$masa_kerja_akhir){					
					$masa_kerja 			= new TbMasakerjaKetentuan;			
					$masa_kerja->masa_kerja = $masa_kerja_awal.'-'.$masa_kerja_akhir;
					$masa_kerja->save();					
				
					Yii::app()->user->setFlash('success','data masa kerja berhasil disimpan.');
				}				
				else Yii::app()->user->setFlash('error','data masa kerja tidak berhasil disimpan.');
			}
			else Yii::app()->user->setFlash('error','data masa kerja tidak berhasil disimpan.');
		}
		
		$this->render('add_masa_kerja',array('url'=>$url,'id'=>$id,'from'=>$from));
	}

	public function ActionAdd_golongan(){
		$this->state 	= 'Tambah Golongan';
		$url 			= 'TbKetentuan/create?action=create';
		$id 			= '';
		$from 			= 'create';
		...
		if(isset($_REQUEST['from'])){						
			if($_REQUEST['golongan']!=''){		
				$golongan 					= new TbGolongan;			
				$golongan->Nama_golongan 	= $_REQUEST['golongan'];
				$golongan->save();					

				Yii::app()->user->setFlash('success','data golongan berhasil disimpan.');
			}
			else Yii::app()->user->setFlash('error','data golongan tidak berhasil disimpan.');
		}
		
		$this->render('add_golongan',array('url'=>$url,'id'=>$id,'from'=>$from));
	}
