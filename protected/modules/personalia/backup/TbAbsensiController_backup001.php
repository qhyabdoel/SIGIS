...
class TbAbsensiController extends Controller{
	...
	public function actionCheck(){				
		$jam_keluar 		= TbAbsensi::model()->find()->Jam_Keluar;
	...
	public function actionEksport_per_karyawan(){
		$query = 'select Tanggal, Jam_Masuk, Jam_Keluar, Total_Jam_Kerja 
	...
	public function actionVerifikasi(){
		...
		if(isset($_REQUEST['Cuti'])){
			...
			if($tanggalIjin!=''){				
				...
				$condition 				= 'Tanggal_Awal like "_____'.$bulanIjin.'___"';
				$ijins 					= Ijin::model()->findAll(array('condition'=>$condition));
				$condition 				= 'Tanggal_Awal like "_____'.$bulanTerlambat.'___"';
				$terlambats 			= Terlambat::model()->findAll(array('condition'=>$condition));
		...
		if(isset($_REQUEST['nik'])){		
			...
			if(count($karyawan)!=0){				
				...
				if(isset($_REQUEST['action']) and $_REQUEST['action']=='submit'){
					if($active_tab==0){										
						$cuti->save();
						if(count($cuti->getErrors())==0) 
							Yii::app()->user->setFlash('success','Data cuti telah tersimpan');
					}
					elseif($active_tab==1){						
						$ijin->save();						
						if(count($ijin->getErrors())==0) 
							Yii::app()->user->setFlash('success','Data ijin telah tersimpan.');
					}
					elseif($active_tab==2){
						$sakit->save();
